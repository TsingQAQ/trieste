...
def test_hvprobability_of_improvement_builder_builds_hvprobability_of_improvement(
    query_at: tf.Tensor, dataset: Mapping, nadir_point: tf.Tensor
) -> None:
    pareto = Pareto(dataset)
    models = {
        "OBJECTIVE{}".format(i): QuadraticWithUnitVariance() for i in range(len(dataset.keys()))
    }
...
@pytest.mark.parametrize(
    "obj_num, pareto, at, expected, nadir",
    [
        (
            2,
            Pareto({"OBJECTIVE": Dataset(tf.ones((1, 1)), tf.constant([[0.5, 0.5]]))}),
            tf.constant([[0.0]]),
            tf.constant([[1.5283500097854048]]),
            tf.constant([[2.0, 2.0]]),
        ),
        (
            5,
            Pareto({"OBJECTIVE": Dataset(tf.ones((1, 1)), tf.constant([[3.0] * 5]))}),
            tf.constant([[1.0, 1.0]]),
            tf.constant([[27.629129665031567]]),
            tf.constant([[4.0] * 5]),
        ),
    ],
)
def test_hvprobability_of_improvement(obj_num, pareto, at, expected, nadir) -> None:
    actual = hv_probability_of_improvement(
        {"OBJECTIVES{}".format(i): QuadraticWithUnitVariance() for i in range(obj_num)},
        at,
        pareto,
        nadir,
    )
